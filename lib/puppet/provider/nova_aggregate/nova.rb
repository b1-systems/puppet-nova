...
Puppet::Type.type(:nova_aggregate).provide(
  ...
  def metadata=(val)
    ...
    id = self.class.nova_aggregate_resources_get_name_by_id(name)
    attrs = self.class.nova_aggregate_resources_attr(id)
    #get keys which are in val but not in current metadata
    new_keys = val.keys - attrs['Metadata'].keys
    #get keys which are in current metadata but not in val
  ...
  def availability_zone=(val)
    id = self.class.nova_aggregate_resources_get_name_by_id(name)
    #this also sets the availability_zone in metadata and deletes all other metadata! 
    auth_nova("aggregate-update", id, name, "#{val}")

    #now restore the old metadata
    @resource[:metadata].each do |key, value|
      set_metadata_helper(id, key, value)
    end
  end
