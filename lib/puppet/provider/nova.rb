...
correctly configured.")
  ...
  def self.cliout2list(output)
    ...
    output.each_line do |line|
      ...
      if not line.match("^\\+")
        ...
        #We replace 'None' because the grizzly version use None instead of '-'
        #This means that we never should allow a Name/Availability Zone called 'None'
        line = line.split("|").map do |el|
          el.strip().gsub(/^-$/, "").gsub(/^None$/, "")
        end
  ...
  def self.nova_aggregate_resources_attr(id)
    ...
    return cliout2list(cmd_output)[0]
  end
